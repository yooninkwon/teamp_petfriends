$(document).ready(function() {
	...
	// 관심사 데이터를 테이블에 반영하는 함수
	function updateCareInfoTable(petCareData) {
		const careInterests = ["관절", "모질", "소화기", "눈", "눈물", "체중", "치아", "피부", "신장", "귀", "심장", "호흡기"];
		let interestCounts = {};

		// 관심사 목록 초기화
		careInterests.forEach(interest => {
			interestCounts[interest] = 0;
		});
		let otherCount = 0;

		// 데이터 카운트 로직
		petCareData.forEach(entry => {
		    if (entry && entry !== "(null)") {
		        // 쉼표로 구분하고 공백 제거
		        const interests = entry.split(',').map(item => item.trim());
		        interests.forEach(interest => {
		            if (interest === 'X') {
		                // 'X'는 무시하고 다음 항목으로 이동
		                return;
		            }
		            if (careInterests.includes(interest)) {
		...
		// 테이블에 데이터 반영
		const tableRow = $('#careTable').find('tr').eq(1); // 두 번째 행에 데이터 삽입
		tableRow.empty(); // 기존 데이터를 초기화한 후 다시 채움
		careInterests.forEach(interest => {
			tableRow.append(`<td>${interestCounts[interest]}</td>`);
		});
		tableRow.append(`<td>${otherCount}</td>`); // 기타 항목 추가
	}
	...
	function fetchPetData(petType) {
		$.ajax({
			...
			success: function(response) {
				...
				const petCareData = response.map(item => item.pet_care);
				...
				// 관심사 테이블 업데이트
				updateCareInfoTable(petCareData);
			},
			error: function(error) {
