...
public class PethotelReserveService implements HelppetfExecuteModelRequestSession<String> {
	...
	public void setFormList(ArrayList<PethotelFormDataDto> formList) {
	...
	public ResponseEntity<String> execute(Model model, HttpServletRequest request, HttpSession session) {
		...
		// 예약번호를 시작일[0~2] + 종료일[0~2] + 멤버아이디[0] + 랜덤숫자(0~9999) 로 설정 => ex)
		// 2024110620241206_dpoowa
		String hph_reserve_no = memStartDate[0] + memStartDate[1] + memStartDate[2] + memEndDate[0] + memEndDate[1]
				+ memEndDate[2] + memId[0] + random;
		...
		try {
			...
			for (int i = 0; i < formList.size(); i++) {
				...
				// 파라미터로 첨부하여 DB에 저장
				helpDao.pethotelReverseRequestPets(hph_reserve_no, hphp_reserve_pet_no, hphp_pet_name, hphp_pet_type,
						hphp_pet_birth, hphp_pet_gender, hphp_pet_weight, hphp_pet_neut, hphp_pet_comment);
			}
			...
			// 파라미터로 첨부하여 DB에 저장
			helpDao.pethotelReverseRequestMem(hph_reserve_no, mem_code, mem_nick, hph_numof_pet, hph_start_date,
					hph_end_date);
		} catch (Exception e) {
		...
		try {
			...
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("JSON 처리 중 오류 발생");
		}
